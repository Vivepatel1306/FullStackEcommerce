...
For Neon, set `DATABASE_URL` to the pooled connection string from Neon and `DIRECT_URL` to the non-pooler connection string. Both URLs should include `sslmode=require`. Run migrations with `npx prisma migrate dev` or deploy them with `npx prisma migrate deploy`.
